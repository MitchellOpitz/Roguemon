...
public class PartyManager : MonoBehaviour
{
    ...
    {
        StartCoroutine(AddStartingPetWithDelay(3f));
        StartCoroutine(AddStartingPetWithDelay(6f));
        StartCoroutine(AddStartingPetWithDelay(9f));
    }
    ...
    public void AddToParty(GameObject pet)
    {
        ...
        Vector3 spawnPosition = party[party.Length - 1].transform.position + spawnDirection * 2f;
    ...
    private void UpdatePartyMovement()
    {
        for (int i = 0; i < party.Length; i++)
        {
            ...
            else
            {
                ...
                follow.enabled = false;  // True later.
